...
public final class QueryUtils {
    ...
    public static List<News> extractNews(String jsonResponse) {
        ...
        try {
            ...
            JSONObject baseJsonReponse= new JSONObject(jsonResponse);
            JSONObject responseJson=baseJsonReponse.getJSONObject("response");
            ...
            for(int i=0;i<newsArray.length();i++){
                ...
                String mSection=results.getString("sectionName");
                String webUrl=results.getString("webUrl");
                News currentNews=new News(mTitle,mSection,webUrl);
                news.add(currentNews);
            }
            newsArray= responseJson.getJSONArray("leadContent");
            for(int i=0;i<newsArray.length();i++){
                JSONObject results= newsArray.getJSONObject(i);
                //   JSONObject properties= currentEarthquake.getJSONObject("properties");
                String mTitle=results.getString("webTitle");
